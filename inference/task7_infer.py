...
if __name__=='__main__':
    ...
    with jsonlines.open(test_path,'r') as data:
        for line in data:
            if line['task']=='7':
                task7_questions.append(line)
    prompts = []
    for question in task7_questions:
    ...
    lora_path = '/bohr/sci-lora-model-q9h7/v14/yi-lora-task7-0831'
    ...
    task7_answer = []
    for i, obj in enumerate(task7_questions):
        obj['ideal'] = ans[i]
        task7_answer.append(obj)
    with jsonlines.open('task7_answer.jsonl', 'w') as file:
        file.write_all(task7_answer)