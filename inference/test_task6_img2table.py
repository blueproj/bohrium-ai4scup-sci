...
def task6_vl_chat(img_path, question=None):
    '''
    task6,利用vl从图片中抽取表格
    '''
...
def image2table_1(match, base_path):
    ...
    new_content = task6_vl_chat(f'{base_path}/{image_url}').replace('\n\n', '\n')
    return f'![{alt_text}](({image_url}){new_content})'
...
def task6_process():
    ...
    for subdir in os.listdir(base_path):
        ...
        if os.path.exists(f'{md_dir}/new_{subdir}.md'):
            ...
            new_content2 = re.sub(r'((?:Table|TABLE).{1,300}\n{1,2}!\[.*\])\((images/.*\.jpg)\)', replace_image_1, new_content1)
